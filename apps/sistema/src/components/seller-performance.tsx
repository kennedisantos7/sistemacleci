...
import type { SellerDashboard } from "@/server/services/seller-dashboard";
...
/**
 * Corpo do painel de um vendedor: números do período, evolução, meta, melhores
 * clientes e últimos documentos.
 *
 * Serve às duas telas — o próprio vendedor em /vendedor e o administrador em
 * /admin/vendedores/[id]. Os dados são exatamente os mesmos (o service é o
 * mesmo); só a VOZ muda, porque "sua carteira" não faz sentido para quem está
 * olhando o time. Manter uma cópia só é o que impede as duas divergirem.
 */
export type Voz = "propria" | "admin";

export function SellerPerformance({
  d,
  voz,
  /** Nome curto do vendedor. Usado só na voz de admin. */
  nome,
}: {
  d: SellerDashboard;
  voz: Voz;
  ...
  const admin = voz === "admin";
  const quem = admin ? (nome ?? "O vendedor") : "Você";
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <StatCard
          title={admin ? "Comissão do período" : "Comissão a receber"}
          ...
          hint={
            d.comissao.configurada
              ...
              : admin
                ? "Percentual ainda não definido para este vendedor"
      ...
      {d.carteira.liberandoEmBreve > 0 ? (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <strong>
            ...
            {d.carteira.liberandoEmBreve === 1 ? "" : "s"} d{admin ? "e" : "a sua"} carteira
            {admin ? ` de ${quem}` : ""}
          </strong>{" "}
          {d.carteira.liberandoEmBreve === 1 ? "fica livre" : "ficam livres"} para outros vendedores
          em até {d.carteira.diasAlerta} dias sem atividade.{" "}
          {admin ? null : (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Valor finalizado por dia · {d.range.label}</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {d.orcamentosRecentes.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              {admin ? `${quem} ainda não criou orçamentos.` : "Você ainda não criou orçamentos."}
            </p>
          ) : (
